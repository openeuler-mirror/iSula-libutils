			      ...
			      struct lxc_log_event *event)
{
	...
	return isula_libutils_write_nointr(fd_to_use, buffer, n + 1);
}
...
static int open_fifo(const char *fifo_path)
{
    ...
    fifo_fd = isula_libutils_open(fifo_path, O_RDWR | O_NONBLOCK, 0);
    if (fifo_fd == -1) {
...
int isula_libutils_log_enable(const struct isula_libutils_log_config *log)
{
	...
	if (log->file) {
        ...
        if (isula_libutils_build_dir(log->file) != 0) {
        ...
        log_fname = isula_libutils_strdup_s(log->file);
	}
